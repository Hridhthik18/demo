...
ALTER TABLE job_history ADD FOREIGN KEY (department_id) REFERENCES departments(department_id);DROP SCHEMA
...
-- task2:
select department_id,first_name,last_name,salary from employees where salary > (select avg(salary) from employees) ;
...
select avg(salary) from employees where department_id in(select department_id from departments where department_name='sales');
...
(select(sum(salary)*60)/100 as 'percentage' from employees as el where el.department_id=e.department_id);